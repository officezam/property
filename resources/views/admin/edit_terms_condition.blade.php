...
<!--                            Terms & Condition
                        ...
                        <!-- /. tools --> </div>
                    <!-- /.box-header -->
                    @foreach($terms_condition as $record)
                    <div class='box-body pad'>
                        <form method="POST" action="{{ URL::action('AboutUsController@update_terms_condition',['id' => $record]) }}">
                            {{ csrf_field() }}

                            <textarea class="textarea editor-cls" placeholder="Place some text here" style="height:500px" value="" id="description" name="description">{{ $record->description}}</textarea>
